  ...
  if (opts.isHeader) {
    html += "<thead><tr>".concat(columns.map(function (column) {
      ...
      if (column.type === 'checkbox' || column.type === 'selection') {
        return "<th class=\"".concat(classNames.join(' '), "\"><div ").concat(isPrint ? '' : "style=\"width: ".concat(column.renderWidth, "px\""), "><input type=\"checkbox\" class=\"").concat(allCls, "\" ").concat(isAllSelected ? 'checked' : '', "><span>").concat(_tools.UtilTools.formatText(cellTitle, true), "</span></div></th>");
      }
